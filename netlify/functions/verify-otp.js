...
export async function handler(event) {
  ...
  try {
    ...
    const { data: rec, error: fe } = await supabase.from("phone_otps").select("*")
      .eq("phone", normalizedPhone).eq("otp_code", String(otp).trim()).eq("verified", false)
    ...
    if (si?.user) userId = si.user.id;
    else {
      const { data: su, error: se } = await supabase.auth.admin.createUser({
        email: fakeEmail, password: tempPassword, email_confirm: true,
    ...
    await supabase.from("user_profiles").upsert({ id: userId, email: fakeEmail, phone: normalizedPhone, phone_verified: true, username: normalizedPhone }, { onConflict: "id" });

    return { statusCode: 200, headers, body: JSON.stringify({ success: true, verified: true, phone: normalizedPhone, userId, fakeEmail, tempPassword }) };
